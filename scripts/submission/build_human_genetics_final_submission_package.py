...
def build_tables(d: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    ...
    table1 = pd.DataFrame(
        [
            ...
            ["Eligible non-DAR retinal OCR comparator", "Retinal comparator", "Retinal OCR SNPs retained after exclusion of DAR-overlapping intervals and interval-level comparator filtering", "379,279 analysis-overlap SNPs", "Primary eligible retinal-regulatory comparator; see Methods for the 1,069/163/267 set identity"],
        ],
    ...
    table2 = pd.DataFrame(table2_rows, columns=["Endpoint", "DAR events / N (%)", "Eligible non-DAR OCR events / N (%)", "OR", "95% CI", "P"])
...
def read_original_sections() -> dict[str, str]:
    ...
    text = text.replace("Large refractive-error GWAS have identified many susceptibility loci, and the most recent multi-ancestry study of EUR, EAS and AFR cohorts greatly expanded discovery while reporting broad cross-ancestry sharing (Verhoeven et al. 2013; Hysi et al. 2020; Cheng et al. 2026).", "Large refractive-error GWAS have substantially expanded locus discovery (Hysi et al. 2020). Earlier multi-ancestry studies further expanded discovery across populations (Verhoeven et al. 2013), while Cheng et al. (2026) reported substantial cross-ancestry sharing alongside ancestry-enriched or ancestry-specific signals.")
    text = text.replace("Large refractive-error GWAS have substantially expanded locus discovery (Hysi et al. 2020). Multi-ancestry studies have subsequently demonstrated substantial cross-ancestry sharing while identifying locally ancestry-enriched or ancestry-specific signals (Verhoeven et al. 2013; Cheng et al. 2026). That shared background does not remove the need to understand local differences. The same source study also reported ancestry-enriched or ancestry-specific association signals, consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.", "Large refractive-error GWAS have substantially expanded locus discovery (Hysi et al. 2020). Earlier multi-ancestry studies further expanded discovery across populations (Verhoeven et al. 2013), while Cheng et al. (2026) reported substantial cross-ancestry sharing alongside ancestry-enriched or ancestry-specific signals. That shared background does not remove the need to understand local differences, consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.")
    ...
    text = text.replace("The final analysis counts were 380,615 SNPs in all retinal OCRs, 1,232 SNPs in ancestry-associated DARs and 379,279 SNPs in matched non-DAR retinal OCRs.", "The final analysis counts were 380,615 SNPs in all retinal OCRs, 1,232 SNPs in ancestry-associated DARs and 379,279 SNPs in the eligible non-DAR retinal OCR comparator. Of the 1,232 ancestry-associated DAR SNPs, 1,069 overlapped the broad retinal OCR annotation, whereas 163 source-defined DAR SNPs fell outside the union-OCR annotation. Eligible non-DAR retinal OCR comparator SNPs were defined after DAR exclusion and interval-level comparator filtering, which excluded 267 additional OCR non-DAR SNPs.")
    text = text.replace("The ancestry-associated DAR annotation was treated as a regulatory context for statistical testing, not as a causal mechanism label. The matched non-DAR retinal OCR set was used as an operational comparator and was not interpreted as proof of regulatory invariance across ancestries. Cell-type-specific S-LDXR, locus prioritization, CRE-to-gene mapping and motif analyses were not performed.", "The ancestry-associated DAR annotation was treated as a regulatory context for statistical testing, not as a causal mechanism label. The eligible non-DAR retinal OCR comparator was used for the primary ancestry-associated DAR enrichment test.")
    text = text.replace("The ancestry-associated DAR annotation was treated as a regulatory context for statistical testing, not as a causal mechanism label. The matched non-DAR retinal OCR set was used as an operational comparator for ancestry-associated DAR enrichment tests.", "The ancestry-associated DAR annotation was treated as a regulatory context for statistical testing, not as a causal mechanism label. The eligible non-DAR retinal OCR comparator was used for the primary ancestry-associated DAR enrichment test.")
    ...
    text = text.replace("This design controls for broad retinal regulatory context and targets the ancestry-DAR label itself.", "The comparator places the primary test within a retinal-regulatory background, and the matched permutation analysis further conditions on OCR membership and genomic characteristics.")
    text = text.replace("This design provides a retinal-regulatory background comparator, reduces confounding by broad OCR membership and targets the ancestry-DAR label itself.", "The comparator places the primary test within a retinal-regulatory background, and the matched permutation analysis further conditions on OCR membership and genomic characteristics.")
    text = text.replace("a matched heterogeneity test comparing ancestry-associated differentially accessible retinal regions (DARs) with non-DAR retinal OCRs.", "an association-effect heterogeneity analysis comparing ancestry-associated differentially accessible retinal regions (DARs) with eligible non-DAR retinal OCRs, supplemented by matched permutation testing.")
    text = text.replace("In the primary DAR analysis, 71 of 1,232 DAR SNPs and 19,653 of 379,279 matched comparator SNPs", "In the primary DAR analysis, 71 of 1,232 DAR SNPs and 19,653 of 379,279 eligible non-DAR retinal OCR comparator SNPs")
    text = text.replace("relative to a tissue-matched non-DAR retinal regulatory background", "relative to an eligible non-DAR retinal OCR background")
    text = text.replace("matched non-DAR retinal OCR SNPs", "eligible non-DAR retinal OCR comparator SNPs")
    text = text.replace("matched non-DAR retinal OCRs", "eligible non-DAR retinal OCRs")
    text = text.replace("matched non-DAR retinal OCR", "eligible non-DAR retinal OCR comparator")
    text = text.replace("matched comparator event rate", "eligible comparator event rate")
    text = text.replace("eligible matched non-DAR retinal OCRs", "eligible non-DAR retinal OCRs")
    text = text.replace("eligible eligible non-DAR retinal OCRs", "eligible non-DAR retinal OCRs")
    text = text.replace("informative strata retained 1,069 DAR SNPs and 59,215 matched non-DAR SNPs", "informative strata retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs")
    text = text.replace("The same source study also reported ancestry-enriched or ancestry-specific association signals, consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.", "This pattern is consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.")
    text = text.replace("That shared background does not remove the need to understand local differences. This pattern is consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.", "That shared background does not remove the need to understand local differences, consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions.")
    text = text.replace("The tissue-matched comparator is central to that test.", "The retinal comparator is central to that test.")
    text = text.replace("The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs; informative strata retained 1,069 DAR SNPs and 59,215 matched non-DAR SNPs.", "The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs. Because 163 source-defined DAR SNPs fell outside the broad OCR annotation, OCR status was retained as a stratification variable; only strata containing both annotation labels contributed to the informative permutation set, which retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs.")
    text = text.replace("The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs; informative strata retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs.", "The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs. Because 163 source-defined DAR SNPs fell outside the broad OCR annotation, OCR status was retained as a stratification variable; only strata containing both annotation labels contributed to the informative permutation set, which retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs.")
...
def final_manuscript_md(tables: dict[str, pd.DataFrame]) -> str:
    ...
    legends = "\n\n".join(
        [
            "Fig. 1. Study design. Study framework with two separate input streams. Public EUR no-23andMe and EAS refractive-error GWAS summary statistics entered GWAS harmonization, including allele alignment, shared-variant matching, reference anchoring and paired-reference checks. Human retinal regulatory atlas annotations entered a separate regulatory-processing stream, including source annotation definition, coordinate harmonization/liftover, reference anchoring and SNP annotation mapping. The streams were joined in an aligned analysis framework used to test cross-ancestry genetic sharing genome-wide and within broad retinal OCRs, and ancestry-linked association-effect heterogeneity in ancestry-associated DARs relative to eligible non-DAR retinal OCR comparator SNPs. Robustness checks were attached to the DAR heterogeneity branch.",
    ...
    md = "\n".join(out)
    polish_map = {
        "Ancestry-associated molecular regulatory variation is increasingly measurable in human tissues, but its relationship to ancestry-divergent genetic effects on complex traits remains unclear. We tested whether ancestry-associated retinal regulatory variation marks genomic regions with divergent association effects between EUR and EAS refractive-error GWAS. Public EUR no-23andMe and EAS refractive-error GWAS summary statistics were harmonized, reference-anchored and analyzed with ancestry-matched LD score regression, cross-population S-LDXR, human retinal open chromatin regions (OCRs) and an association-effect heterogeneity analysis comparing ancestry-associated differentially accessible retinal regions (DARs) with eligible non-DAR retinal OCRs, supplemented by matched permutation testing. The analysis set included 3,112,573 shared SNPs after effect alignment and paired-reference checks. In primary S-LDXR analyses using MAF > 0.05 in both ancestries, genome-wide S-LDXR estimated high EUR–EAS sharing (GCORSQ = 1.028, SE = 0.100), and broad retinal OCRs showed a compatible pattern (GCORSQ = 0.948, SE = 0.097). In the primary DAR analysis, 71 of 1,232 DAR SNPs and 19,653 of 379,279 eligible non-DAR retinal OCR comparator SNPs fell in the top 5% heterogeneity endpoint (OR = 1.119, 95% CI 0.867–1.423, P = 0.367; matched permutation P = 0.334). Ancestry-associated retinal regulatory variation was not accompanied by a detectable large systematic excess of EUR–EAS association-effect heterogeneity, although modest enrichment remains possible.":
        "Ancestry-associated molecular regulatory variation is increasingly measurable in human tissues, but its relationship to ancestry-divergent genetic effects on complex traits remains unclear. We tested whether ancestry-associated retinal regulatory variation marks genomic regions with divergent association effects between EUR and EAS refractive-error GWAS. Public EUR no-23andMe and EAS summary statistics were harmonized, reference-anchored and analyzed with ancestry-matched LD score regression, cross-population S-LDXR and human retinal open chromatin regions (OCRs). We then compared ancestry-associated differentially accessible retinal regions (DARs) with eligible non-DAR retinal OCRs in an association-effect heterogeneity analysis, supplemented by matched permutation testing. The analysis set included 3,112,573 shared SNPs after effect alignment and paired-reference checks. In primary S-LDXR analyses using MAF > 0.05 in both ancestries, genome-wide S-LDXR estimated high EUR–EAS sharing (GCORSQ = 1.028, SE = 0.100), and broad retinal OCRs showed a compatible pattern (GCORSQ = 0.948, SE = 0.097). In the primary DAR analysis, 71 of 1,232 DAR SNPs and 19,653 of 379,279 eligible non-DAR retinal OCR comparator SNPs fell in the top 5% heterogeneity endpoint (OR = 1.119, 95% CI 0.867–1.423, P = 0.367; matched permutation P = 0.334). Ancestry-associated retinal regulatory variation was not accompanied by a detectable large systematic excess of EUR–EAS association-effect heterogeneity, although modest enrichment remains possible.",
        "Refractive error is common, highly polygenic and a major global visual-health burden, making it a strong model for studying cross-ancestry complex-trait architecture. Large refractive-error GWAS have substantially expanded locus discovery (Hysi et al. 2020). Earlier multi-ancestry studies further expanded discovery across populations (Verhoeven et al. 2013), while Cheng et al. (2026) reported substantial cross-ancestry sharing alongside ancestry-enriched or ancestry-specific signals. That shared background does not remove the need to understand local differences, consistent with the principle that locus discovery and cross-ancestry effect sharing are distinct questions. Allele frequency, LD and tagging can change association signals even when effects are conserved, and fine-scale ancestry work shows that effect-size conservation can coexist with reduced portability and local ancestry-dependent patterns (Hu et al. 2025). The key question is therefore not simply whether ancestry-related association differences exist, but where association-effect heterogeneity is concentrated and whether it follows biologically relevant regulatory variation.":
        "Refractive error is common, highly polygenic and a major global visual-health burden, making it a strong model for studying cross-ancestry complex-trait architecture. Large refractive-error GWAS have substantially expanded locus discovery (Hysi et al. 2020). Earlier multi-ancestry studies extended discovery across populations (Verhoeven et al. 2013), and Cheng et al. (2026) reported substantial cross-ancestry sharing together with ancestry-enriched or ancestry-specific signals. These findings show that locus discovery and cross-ancestry effect sharing are distinct questions. Allele frequency, LD and tagging can change association signals even when effects are conserved. Fine-scale ancestry work also shows that effect-size conservation can coexist with reduced portability and local ancestry-dependent patterns (Hu et al. 2025). The remaining question is where association-effect heterogeneity is concentrated and whether it follows biologically relevant regulatory variation.",
        ...
        "The retina is directly relevant to refractive development and visually guided eye growth. Experimental and genetic studies support retinal contributions to refractive-error biology, but GWAS alone rarely shows whether ancestry-linked regulatory differences mark divergent trait effects (Wallman et al. 1987; Kiefer et al. 2013; Hysi et al. 2020). The Human Retina Cell Atlas adds the required tissue context by mapping single-cell transcriptomes and chromatin accessibility across retinal cell classes and reporting ancestry-associated regulatory variation (Li et al. 2026). This atlas shows that ancestry-associated retinal molecular variation exists. It does not show whether those differences translate into ancestry-divergent association effects. The atlas therefore enables a direct test of whether ancestry-associated retinal chromatin differences correspond to ancestry-divergent genetic effects for refractive error.",
        "We tested whether ancestry-associated retinal regulatory variation predicts where cross-ancestry refractive-error association effects diverge. The analysis used a three-level nested design. First, we estimated whether EUR–EAS refractive-error common-variant architecture is broadly shared genome-wide. Second, we asked whether this shared pattern extends to broad retinal open chromatin regions. Third, we tested whether ancestry-associated retinal differentially accessible regions preferentially concentrate association-effect heterogeneity relative to an eligible non-DAR retinal OCR background. The key innovation is testing whether ancestry-associated regulatory variation predicts where cross-ancestry trait-effect divergence occurs. This nested design distinguishes broad cross-ancestry genetic sharing from the narrower question of whether ancestry-associated retinal accessibility marks identify regions of effect divergence.":
        "We tested whether ancestry-associated retinal regulatory variation predicts where cross-ancestry refractive-error association effects diverge. The analysis used a three-level nested design. First, we estimated whether EUR–EAS refractive-error common-variant architecture is broadly shared genome-wide. Second, we tested whether this shared pattern extends to broad retinal open chromatin regions. Third, we assessed whether ancestry-associated retinal differentially accessible regions preferentially concentrate association-effect heterogeneity relative to an eligible non-DAR retinal OCR background. This design separates broad cross-ancestry genetic sharing from the narrower question of whether ancestry-associated retinal accessibility marks regions of effect divergence.",
        "The study used publicly available ancestry-specific refractive-error GWAS summary statistics from Cheng et al. (2026). The source study reported ancestry-stratified and cross-ancestry meta-analyses for refractive error in people of European (EUR; n = 1,495,159), East Asian (EAS; n = 121,172) and African (AFR; n = 144,737) ancestries. The EUR source study included a 23andMe component of 106,086 cases and 85,757 controls. The publicly released EUR summary statistics used here excluded 23andMe participants; the released file provided variant-level effective sample-size values, which were retained as the analysis sample-size information rather than replacing them with the full source EUR total. The EAS public file also included a variant-level `N` field, and the source EAS total was treated as source-study metadata rather than a constant analysis N. The present analysis focused on the EUR–EAS comparison because this was the specified inference unit for paired LD-based S-LDXR and SNP-level heterogeneity testing.":
        "The study used publicly available ancestry-specific refractive-error GWAS summary statistics from Cheng et al. (2026). The source study reported ancestry-stratified and cross-ancestry meta-analyses for refractive error in people of European (EUR; n = 1,495,159), East Asian (EAS; n = 121,172) and African (AFR; n = 144,737) ancestries. The EUR source study included a 23andMe component of 106,086 cases and 85,757 controls. The publicly released EUR summary statistics used here excluded 23andMe participants. The released file provided variant-level effective sample sizes, which were retained as the analysis sample-size information rather than replaced with the full source EUR total. The EAS public file also included a variant-level `N` field, so the source EAS total was treated as metadata rather than a constant analysis N. The present analysis focused on the EUR–EAS comparison because this was the specified inference unit for paired LD-based S-LDXR and SNP-level heterogeneity testing.",
        "The final analysis counts were 380,615 SNPs in all retinal OCRs, 1,232 SNPs in ancestry-associated DARs and 379,279 SNPs in the eligible non-DAR retinal OCR comparator. Of the 1,232 ancestry-associated DAR SNPs, 1,069 overlapped the broad retinal OCR annotation, whereas 163 source-defined DAR SNPs fell outside the union-OCR annotation. Eligible non-DAR retinal OCR comparator SNPs were defined after DAR exclusion and interval-level comparator filtering, which excluded 267 additional OCR non-DAR SNPs.":
        "The final analysis included 380,615 SNPs in all retinal OCRs, 1,232 SNPs in ancestry-associated DARs and 379,279 SNPs in the eligible non-DAR retinal OCR comparator. Among the 1,232 ancestry-associated DAR SNPs, 1,069 overlapped the broad retinal OCR annotation and 163 source-defined DAR SNPs fell outside the union-OCR annotation. Eligible non-DAR retinal OCR comparator SNPs were defined after DAR exclusion and interval-level comparator filtering. This filtering excluded 267 additional OCR non-DAR SNPs.",
        ...
        "Available cohort documentation did not identify material cross-ancestry participant overlap. Covariance was therefore set to zero, while residual overlap remains a possible source of uncertainty.",
        "The matched permutation analysis used 1,000 permutations with random seed 20260912. DAR/non-DAR annotation labels were permuted within strata defined by chromosome, average EUR/EAS reference MAF decile, absolute EUR–EAS MAF-difference quintile, baseline LD-score decile and retinal OCR status. The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs. Because 163 source-defined DAR SNPs fell outside the broad OCR annotation, OCR status was retained as a stratification variable; only strata containing both annotation labels contributed to the informative permutation set, which retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs. The empirical P value was calculated as (1 + number of null statistics with absolute value greater than or equal to the observed absolute statistic) / (1 + number of permutations). The observed statistic was the mean χ²ₕₑₜ difference between ancestry-associated DAR SNPs and eligible non-DAR retinal OCR comparator SNPs.":
        "The matched permutation analysis used 1,000 permutations with random seed 20260912. DAR/non-DAR annotation labels were permuted within strata defined by chromosome, average EUR/EAS reference MAF decile, absolute EUR–EAS MAF-difference quintile, baseline LD-score decile and retinal OCR status. The permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs. OCR status was retained as a stratification variable because 163 source-defined DAR SNPs fell outside the broad OCR annotation. Only strata containing both annotation labels contributed to the informative permutation set, which retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs. The empirical P value was calculated as (1 + number of null statistics with absolute value greater than or equal to the observed absolute statistic) / (1 + number of permutations). The observed statistic was the mean χ²ₕₑₜ difference between ancestry-associated DAR SNPs and eligible non-DAR retinal OCR comparator SNPs.",
        "The primary estimate was further evaluated using effect-size precision and power. The primary top 5% odds ratio was 1.119, with a 95% CI of 0.867–1.423. With 1,232 DAR SNPs and a eligible comparator event rate of 5.18%, the minimum detectable odds ratio was 1.397 for 80% power and 1.467 for 90% power at α = 0.05. Using OR = 1.5 as a prespecified robustness boundary, the confidence interval excluded an enrichment as large as OR = 1.5 under the selected endpoint. The same analysis did not exclude modest enrichment. The most accurate conclusion is that the data do not support a large systematic excess of ancestry-divergent effects in ancestry-associated retinal DARs, while smaller effects remain unresolved.":
        "The primary estimate was further evaluated using effect-size precision and power. The primary top 5% odds ratio was 1.119, with a 95% CI of 0.867–1.423. With 1,232 DAR SNPs and an eligible comparator event rate of 5.18%, the minimum detectable odds ratio was 1.397 for 80% power and 1.467 for 90% power at α = 0.05. Using OR = 1.5 as a prespecified robustness boundary, the confidence interval excluded enrichment as large as OR = 1.5 under the selected endpoint. The analysis did not exclude modest enrichment. The data therefore do not support a large systematic excess of ancestry-divergent effects in ancestry-associated retinal DARs, while smaller effects remain unresolved.",
        ...
        "Our results separate ancestry-associated retinal molecular variation from cross-ancestry divergence in refractive-error association effects. Refractive error showed predominantly shared EUR–EAS common-variant architecture at the genome-wide level and within broad retinal regulatory regions. Ancestry-associated retinal DARs, by contrast, did not identify a large systematic concentration of ancestry-divergent association effects. This pattern addresses the central question of the study: measurable ancestry-associated retinal regulatory variation does not necessarily map onto systematic divergence in common-variant trait effects. The result separates two related layers of variation. One layer captures molecular regulatory differences observed in retinal tissue across sampled ancestry groups. The other captures marginal GWAS association effects for refractive error after allele alignment, reference anchoring and ancestry-matched LD modeling. These layers can coexist without being interchangeable.",
        "The study’s main contribution is the direct test linking these two previously separate observations. Cheng et al. established where refractive-error genetic associations are broadly shared or ancestry enriched. The Human Retina Cell Atlas established where retinal molecular and chromatin features vary with ancestry. The present study asked whether ancestry-associated retinal regulatory regions preferentially mark genomic locations in which refractive-error association effects diverge across ancestries. The retinal comparator is central to that test. Ancestry-associated DAR SNPs were compared with eligible non-DAR retinal OCR comparator SNPs, rather than with the rest of the genome. The comparator places the primary test within a retinal-regulatory background, and the matched permutation analysis further conditions on OCR membership and genomic characteristics. It avoids treating all retinal open chromatin as equivalent to ancestry-associated accessibility, and it avoids attributing a generic retinal regulatory signal to ancestry-associated molecular variation. The nested design separates whether retinal regulatory regions broadly preserve cross-ancestry sharing from the narrower question of whether ancestry-associated accessibility identifies regions of effect divergence.":
        "The study’s main contribution is a direct test linking two previously separate observations. Cheng et al. established where refractive-error genetic associations are broadly shared or ancestry enriched. The Human Retina Cell Atlas established where retinal molecular and chromatin features vary with ancestry. We asked whether ancestry-associated retinal regulatory regions preferentially mark genomic locations where refractive-error association effects diverge across ancestries. The retinal comparator is central to that test. Ancestry-associated DAR SNPs were compared with eligible non-DAR retinal OCR comparator SNPs rather than with the rest of the genome. This comparator places the primary test within a retinal-regulatory background. The matched permutation analysis further conditions on OCR membership and genomic characteristics. Together, these analyses avoid treating all retinal open chromatin as equivalent to ancestry-associated accessibility or attributing a generic retinal regulatory signal to ancestry-associated molecular variation. The nested design separates broad retinal regulatory sharing from the narrower question of whether ancestry-associated accessibility identifies regions of effect divergence.",
        ...
        "The scientific implication is that ancestry-associated molecular annotations should be treated as hypotheses about cross-ancestry trait heterogeneity. They are not, by themselves, evidence of heterogeneity. This distinction matters as multi-ancestry GWAS are increasingly interpreted alongside single-cell ATAC, eQTL, pQTL, spatial-omics and cell-state annotations. An annotation associated with ancestry can be biologically informative, but trait-level effect divergence still needs to be tested directly. The same reasoning applies to genetic prediction. Reduced cross-ancestry prediction portability in refractive error should not be attributed automatically to ancestry-associated retinal chromatin differences. LD, allele frequency, training imbalance, phenotype heterogeneity, local effect differences and environmental structure may all contribute, and this study was not designed to apportion those sources. The precision analysis gives the bounded interpretation of the DAR result. The primary estimate was OR = 1.119, with a 95% CI of 0.867–1.423, so a large OR = 1.5 enrichment was not compatible with the observed interval under the tested endpoint. The confidence interval and power analysis indicate that a large systematic enrichment is unlikely under the tested endpoint, while modest distributed effects remain compatible with the data.",
        "Fig. 1. Study design. Study framework with two separate input streams. Public EUR no-23andMe and EAS refractive-error GWAS summary statistics entered GWAS harmonization, including allele alignment, shared-variant matching, reference anchoring and paired-reference checks. Human retinal regulatory atlas annotations entered a separate regulatory-processing stream, including source annotation definition, coordinate harmonization/liftover, reference anchoring and SNP annotation mapping. The streams were joined in an aligned analysis framework used to test cross-ancestry genetic sharing genome-wide and within broad retinal OCRs, and ancestry-linked association-effect heterogeneity in ancestry-associated DARs relative to eligible non-DAR retinal OCR comparator SNPs. Robustness checks were attached to the DAR heterogeneity branch.":
        "Fig. 1. Study design. Public EUR no-23andMe and EAS refractive-error GWAS summary statistics entered GWAS harmonization, including allele alignment, shared-variant matching, reference anchoring and paired-reference checks. Human retinal regulatory atlas annotations entered a separate regulatory-processing stream, including source annotation definition, coordinate harmonization/liftover, reference anchoring and SNP annotation mapping. The two streams were joined in an aligned analysis framework that tested cross-ancestry genetic sharing genome-wide and within broad retinal OCRs, and association-effect heterogeneity in ancestry-associated DARs relative to eligible non-DAR retinal OCR comparator SNPs. Robustness checks were attached to the DAR heterogeneity branch.",
    }
...
Using public EUR no-23andMe and EAS refractive-error summary statistics, we combined ancestry-matched LD score regression, S-LDXR and retinal regulatory annotations. The results support predominantly shared EUR–EAS common-variant architecture. Genome-wide S-LDXR GCORSQ was 1.028 (SE = 0.100), and broad retinal OCRs showed a compatible estimate of 0.948 (SE = 0.097). In the primary DAR analysis, 71 of 1,232 ancestry-associated DAR SNPs and 19,653 of 379,279 eligible non-DAR retinal OCR comparator SNPs fell in the top 5% association-effect heterogeneity endpoint (OR = 1.119, 95% CI 0.867–1.423, P = 0.367; matched permutation P = 0.334). The confidence interval excluded a large OR = 1.5 enrichment under the tested endpoint, while modest or local effects remain unresolved.

Previous work separately established cross-ancestry refractive-error genetics and ancestry-associated retinal regulatory variation. This study tests whether these two forms of ancestry dependence correspond at the level of association-effect heterogeneity. The result provides a clear framework for interpreting ancestry-associated molecular annotations in cross-ancestry human genetics.
...
### DAR, OCR and comparator set definitions

The broad retinal OCR annotation contained 380,615 analysis-overlap SNPs. The ancestry-associated DAR annotation contained 1,232 analysis-overlap SNPs; 1,069 of these overlapped the broad retinal OCR annotation, whereas 163 source-defined DAR SNPs fell outside the union-OCR annotation. The primary comparator comprised 379,279 eligible non-DAR retinal OCR SNPs retained after DAR exclusion and interval-level comparator filtering. This count follows the identity 380,615 - 1,069 - 267 = 379,279, where 267 denotes additional OCR non-DAR SNPs excluded by interval-level comparator filtering.

### Matched permutation universe

The matched permutation universe contained SNPs annotated as DAR or eligible non-DAR retinal OCRs. DAR/non-DAR annotation labels were permuted within strata defined by chromosome, average EUR/EAS reference MAF decile, absolute EUR–EAS MAF-difference quintile, baseline LD-score decile and retinal OCR status. Because 163 source-defined DAR SNPs fell outside the broad OCR annotation, OCR status was retained as a stratification variable. Only strata containing both annotation labels contributed to the informative permutation set; this retained 1,069 DAR SNPs and 59,215 eligible non-DAR retinal OCR SNPs.
...
def main():
    ...
    (ROOT / "manuscript" / "HUMAN_GENETICS_MANUSCRIPT_FINAL_CONSISTENCY.md").write_text(md, encoding="utf-8")
    ...
    shutil.copy2(SUBMISSION / "HUMAN_GENETICS_MANUSCRIPT_FINAL.docx", ROOT / "manuscript" / "HUMAN_GENETICS_MANUSCRIPT_FINAL_CONSISTENCY.docx")
    make_title_page()
    make_cover_letter()
